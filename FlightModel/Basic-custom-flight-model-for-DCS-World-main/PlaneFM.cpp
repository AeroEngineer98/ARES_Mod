...
#include "include/FM/wHumanCustomPhysicsAPI_ImplementationDeclare.h"	// Provides all DCS related functions in this cpp file. Was "include/FM/API_Declare.h"
// Model headers
...
//The things that move on the model, use the model viewer to learn what each "arg" corresponds to.
{
	...
	//drawargs[25] = (float)limit((PlaneFM::misc_state), 0.0, 1.0); // This is usually the tail hook for most planes with them.
	...
	//drawargs[89] = (float)limit(((PlaneFM::throttle_state - 90.0) / 10.0), 0.0, 1.0);
	//drawargs[90] = (float)limit(((PlaneFM::throttle_state - 90.0) / 10.0), 0.0, 1.0);
